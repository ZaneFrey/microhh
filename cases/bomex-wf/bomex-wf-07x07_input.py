...
# Derive the MicroHH input basename from this script's name. For example,
# bomex-wf-09x09_input.py reads bomex-wf-09x09.ini and writes
# bomex-wf-09x09_input.nc.
input_script = Path(__file__)
script_stem = input_script.stem
if not script_stem.endswith("_input"):
    raise RuntimeError(f"Input generator name must end in '_input.py': {input_script.name}")
ini_name = script_stem[:-len("_input")]
ini_path = input_script.with_name(f"{ini_name}.ini")
nc_path = input_script.with_name(f"{ini_name}_input.nc")
...
nc_file = nc.Dataset(str(nc_path), mode="w", datamodel="NETCDF4", clobber=True)
