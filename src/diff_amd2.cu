...
namespace
{
    ...
    __global__ void reset_aggregates_g(double* counts,const int ncounts,double* maximum)
    {
    ...
    template<typename TF> __global__ void amd_momentum_g(
        TF* evisc,TF* nus,TF* nub,double* counts,double* maximum,
        ...
        const int istart,const int iend,const int jstart,const int jend,const int kstart,const int kend,const int jj,const int kk)
    {
        ...
        const int k=kstart+cell/(nx*ny);
            ...
            if(!finite_A||!isfinite(a)){atomicAdd(&counts[0],1.);return;} if(a==0.){atomicAdd(&counts[1],1.);return;}
            Du=0.;double S[3][3];for(int p=0;p<3;++p)for(int q=0;q<3;++q){Ah[p][q]=A[p][q]/a;Du+=Ah[p][q]*Ah[p][q];}
            if(!isfinite(Du)||!(Du>0.)){atomicAdd(&counts[0],1.);return;}for(int p=0;p<3;++p)for(int q=0;q<3;++q)S[p][q]=0.5*(Ah[p][q]+Ah[q][p]);
            ...
            if(!isfinite(shear)||!isfinite(ns)){atomicAdd(&counts[2],1.);return;}double nb=0.;bool bok=true,zero=false;
            ...
            if(zero)atomicAdd(&counts[10],1.);if(!bok){atomicAdd(&counts[3],1.);return;}if(fabs(ns)>tfmax||fabs(nb)>tfmax){atomicAdd(&counts[4],1.);return;}const double raw=ns+nb;if(!isfinite(raw)){atomicAdd(&counts[5],1.);return;}if(raw<=0.){nus[n]=TF(ns);nub[n]=TF(nb);atomicAdd(&counts[6],1.);return;}const double stored=(cap>0.&&raw>cap)?cap:raw;if(!isfinite(stored)||stored>tfmax){atomicAdd(&counts[7],1.);return;}nus[n]=TF(ns);nub[n]=TF(nb);evisc[n]=TF(stored);status=stored!=raw?9:8;atomicAdd(&counts[status],1.);atomic_max_nonnegative(maximum,stored);
    }

    template<typename TF> __global__ void amd_scalar_g(
        TF* ediff,double* counts,double* maximum,const TF* phi,const TF* u,const TF* v,const TF* w,
...
void Diff_amd2<TF>::exec_viscosity(Stats<TF>&, Thermo<TF>& th)
{
    ...
    cuda_check_error();
    std::vector<double> host_counts(Mom_status_count+1);double host_max;
    cudaMemcpy(host_counts.data(),compact_counts_g,(Mom_status_count+1)*sizeof(double),cudaMemcpyDeviceToHost);
    cudaMemcpy(&host_max,compact_max_g,sizeof(double),cudaMemcpyDeviceToHost);
    Aggregates pass;for(int n=0;n<Mom_status_count;++n){pass.mom[n]=host_counts[n];pass.cells_evaluated+=pass.mom[n];}pass.zero_buoyancy_gradient=host_counts[Mom_status_count];pass.max_evisc=host_max;
    ...
    for(const auto& item:scalar_coeff)
    {
        ...
        cudaMemcpy(host_counts.data(),compact_counts_g,Scalar_status_count*sizeof(double),cudaMemcpyDeviceToHost);
        cudaMemcpy(&host_max,compact_max_g,sizeof(double),cudaMemcpyDeviceToHost);
        auto& c=pass.scalar[item.first];c.resize(Scalar_status_count);for(int n=0;n<Scalar_status_count;++n)c[n]=host_counts[n];pass.max_scalar[item.first]=host_max;
